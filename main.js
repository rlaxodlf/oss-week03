...
const fc = parseForecast(raw);
  ...
  const lat = place.latitude.toFixed(2), lon = place.longitude.toFixed(2);
  console.log(`${chalk.bold(place.name)}, ${place.country} (${lat}, ${lon})`);
  ...
  fc.days.forEach((d)=> {console.log(`${label(d.date)}  min ${d.min.toFixed(1)}  max ${paintMax(d.max)}  ${describe(d.code)}`)});
